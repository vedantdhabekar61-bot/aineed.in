...
const ToolGrid: React.FC<ToolGridProps> = ({ tools, title, loading = false }) => {
  return (
    <section className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-16">
      {title && (
        <div className="flex items-center mb-10 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 sm:gap-8">
        {loading ? (
          // Render 8 shimmer cards while loading
      ...
      {!loading && tools.length === 0 && (
          <div className="text-center py-24 bg-white/50 rounded-3xl border border-slate-100 border-dashed">
              ...
              <p className="text-slate-300 text-sm mt-2">Try a broader description.</p>
          </div>
      )}
    </section>
  );
